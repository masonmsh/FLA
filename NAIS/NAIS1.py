'''
Change based on the NAIS method: adding a two-layer attention (Design1)
...
class NAIS:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            ...
            self.ha = tf.Variable(
                tf.ones([self.weight_size, 1]), name='Ha_for_MLP', dtype=tf.float32)
            self.hb = tf.Variable(tf.ones(
                [self.weight_size, self.embedding_size]), name='Hb_for_MLP', dtype=tf.float32)
    ...
    def _attention_MLP(self, q_):
        with tf.name_scope("attention_MLP"):
            ...
            A_ = tf.reshape(tf.matmul(MLP_output, self.ha), [b, n])
            ...
            exp_A_sum = tf.pow(exp_A_sum, tf.constant(
                self.beta, tf.float32, [1]))

            A = tf.expand_dims(tf.div(exp_A_, exp_A_sum), 2)  # (b, n, 1)

            # (b*n, w) * (w, e) => (None, 1) => (b, n, e)
            B_ = tf.reshape(tf.matmul(MLP_output, self.hb),
                            [b, n, self.embedding_size])
            ...
            return tf.reduce_sum(B * (A * self.embedding_q_), 1)
...
def training(flag, model, dataset,  epochs, num_negatives):
    ...
    with tf.Session() as sess:
        ...
        testDict = evaluate.init_evaluate_model(
            model, sess, dataset.testRatings, dataset.testNegatives, dataset.trainList)
        ...
        for epoch_count in range(epochs):
            ...
            if epoch_count % model.verbose == 0:
                ...
                (hits, ndcgs, losses) = evaluate.eval(model, sess,
                                                      dataset.testRatings, dataset.testNegatives, testDict)
...
if __name__ == '__main__':
    ...
    algo = "n2-concat" if args.algorithm else "n2-prod"
    ...
    logging.basicConfig(filename=os.path.join(log_dir, "%s_%s_%s:%s_%s" % (
        args.dataset, args.embed_size, algo, pret, strftime('%Y-%m-%d-%H:%M:%S', localtime()))), level=logging.INFO)
