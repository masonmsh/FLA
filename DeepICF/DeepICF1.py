'''
Changes based on the DeepICF method: adding a two-layer attention (Design1)
...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='Digital_Music',
...
class DeepICF_a:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            ...
            self.ha = tf.Variable(tf.ones([self.weight_size, 1]), name='Ha_for_MLP', dtype=tf.float32)
            self.hb = tf.Variable(tf.ones([self.weight_size, self.embedding_size]), name='Hb_for_MLP', dtype=tf.float32)
    ...
    def _attention_MLP(self, q_):
        with tf.name_scope("attention_MLP"):
            ...
            A_ = tf.reshape(tf.matmul(MLP_output, self.ha),[b,n]) #(b*n, w) * (w, 1) => (None, 1) => (b, n)
            ...
            exp_A_sum = tf.reduce_sum(exp_A_, 1, keep_dims=True)  # (b, 1)
            exp_A_sum = tf.pow(exp_A_sum, tf.constant(self.beta, tf.float32, [1]))

            A = tf.expand_dims(tf.div(exp_A_, exp_A_sum),2) # (b, n, 1)
            
            
            B_ = tf.reshape(tf.matmul(MLP_output, self.hb),[b,n,self.embedding_size]) #(b*n, w) * (w, e) => (None, 1) => (b, n, e)

            # softmax for not mask features
            exp_B_ = tf.exp(B_) # (b,n,e)
            exp_B_sum = tf.reduce_sum(exp_B_, 2, keep_dims=True)  # (b, 1, 1)

            B = tf.div(exp_B_, exp_B_sum) # (b, n, e)
            B = tf.expand_dims(mask_mat,2) * B

            return B*A,tf.reduce_sum(B * (A * self.embedding_q_), 1)
    ...
    with tf.Session() as sess:
        ...
        for epoch_count in range(epochs):
            ...
            if epoch_count % model.verbose == 0:
                ...
                logging.info("Epoch %d [%.1fs][%.1fs + %.1fs]: HR = %.4f, NDCG = %.4f, loss = %.4f [%.1fs] train_loss = %.4f [%.1fs]" % (
            ...
            if hr > best_hr:
                best_hr = hr
                best_ndcg = ndcg
                saver.save(sess, weight_path+'model.ckpt', global_step=epoch_count)
...
if __name__ == '__main__':
    ...
    algo = "2-concat" if args.algorithm else "2-prod"
    pret = "w" if args.pretrain else "o"

    log_dir = "Log/%s/" % args.dataset
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    logging.basicConfig(filename=os.path.join(log_dir, "%s_%s_%s:%s_%s" %
                                              (args.dataset, args.embed_size, algo, pret,
    ...
    logging.info("End. best HR = %.4f, best NDCG = %.4f" % (best_hr, best_ndcg))
