...
class DeepICF_a:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            ...
            self.h = tf.Variable(tf.ones([self.weight_size, self.embedding_size]), name='H_for_MLP', dtype=tf.float32)
    ...
    def _attention_MLP(self, q_):
        with tf.name_scope("attention_MLP"):
            ...
            A_ = tf.reshape(tf.matmul(MLP_output, self.h), [b, n, self.embedding_size])  # (b*n, w) * (w, e) => (None, 1) => (b, n, e)
...
if __name__ == '__main__':
    ...
    algo = "d1-concat" if args.algorithm else "d1-prod"
